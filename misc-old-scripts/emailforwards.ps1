...
foreach ($User in $allUsers)
{
    ...
    $UserInboxRules += Get-InboxRule -Mailbox $User.UserPrincipalname | Select MailboxOwnerID, Name, Description, Enabled, Priority, ForwardTo, ForwardAsAttachmentTo, RedirectTo, DeleteMessage | Where-Object {($_.ForwardTo -ne $null) -or ($_.ForwardAsAttachmentTo -ne $null) -or ($_.RedirectTo -ne $null)}
...
$UserInboxRules | Export-Csv -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_MailForwardingRulesToExternalDomains.csv" -NoTypeInformation
$UserDelegates | Export-Csv -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_MailboxDelegatePermissions.csv" -NoTypeInformation
$SMTPForwarding | Export-Csv -Path "$((Get-Date).ToString('yyyy-MM-dd_HH-mm-ss'))_Mailboxsmtpforwarding.csv" -NoTypeInformation